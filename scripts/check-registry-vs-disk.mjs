...
const disk = fs.existsSync("plugins") ? fs.readdirSync("plugins", { withFileTypes:true }).filter(d=>d.isDirectory()).map(d=>d.name) : [];

const listed = new Set((reg.plugins||[]).map(p => p.id));
const exists = new Set(disk);

// demos are allowed on disk but never required in registry
const demos = new Set(["fabric.hello-world"]);
