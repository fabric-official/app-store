...
function listChangedPlugins() {
  // If running in CI, rely on git diff; otherwise validate all
  try {
    ...
    files.forEach(f => {
      if (minimatch(f, 'plugins/**')) folders.add(f.split('/').slice(0,2).join('/'));
    });
    return Array.from(folders).length ? Array.from(folders) : fs.readdirSync(path.join(root, 'plugins')).map(p => `plugins/${p}`);
  } catch {
    return fs.readdirSync(path.join(root, 'plugins')).map(p => `plugins/${p}`);
  }
}

const validator = ajv.compile(pluginSchema);
const targets = listChangedPlugins();
...
for (const dir of targets) {
  ...
  const sig = path.join(abs, 'bundle/index.esm.js.sig');
  const sbom = path.join(abs, 'bundle/sbom.json');
  if (!fs.existsSync(bundle)) fail(`Missing bundle: ${bundle}`);
  if (!fs.existsSync(sig)) fail(`Missing signature: ${sig}`);
  if (!fs.existsSync(sbom)) fail(`Missing SBOM: ${sbom}`);
  ok(`Bundle/signature/SBOM present: ${dir}`);
